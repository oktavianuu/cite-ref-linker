...
print("\n" + "="*50)
print("      AUTO-LINKER v3.0 (NARRATIVE SUPPORT)      ")
print("="*50 + "\n")
...
# REGEX 1: Reference List Scanner (Flexible for 'Year' with or without parens)
...
# --- PHASE 2: LINKING (The New Logic) ---
print("\n[*] Phase 2: Linking Citations (Parenthetical & Narrative)")

# REGEX 2: The Combined Citation Finder
# Group 'paren': Matches (Author, 2020)
# Group 'narrative': Matches Author (2020) or Author et al. (2020)
citation_pattern = re.compile(
    r"(?P<paren>\([^\)]+\))|"  # Group A: (Smith, 2020)
    # Group B: Narrative. Perhatikan bagian (?: ... )? di tengah untuk nama kedua
...
for i, p in enumerate(all_paragraphs):
    ...
    text = p.text
    # Pre-check to speed up loop
    if "(" not in text: 
        continue

    # Find matches using the new Combined Regex
    ...
    for match in matches:
        ...
        # 2. Handle Citation (Branch Logic)
        
        # --- CASE A: Narrative Citation "Dikmenli (2010)" ---
        if match.group('narrative'):
            # Parse "Dikmenli (2010)" or "Dikmenli et al. (2010)"
            # Split by '(' to get name and year
            parts = full_text.split('(')
            name_part = parts[0].strip() # "Dikmenli"
            year_part = parts[1].replace(')', '').strip() # "2010"
        ...
        # --- CASE B: Parenthetical Citation "(Dikmenli, 2010)" ---
        elif match.group('paren'):
            content = full_text[1:-1] # Remove parens
...
# --- REPORTING ---
print("\n[*] Saving Document...")
doc.save("output_kusniawati_universal.docx")
