...
def fake_thinking_time():
    """Adds a tiny delay to make it look like the computer is thinking hard."""
    time.sleep(0.005) 
...
print("\n" + "="*50)
print("      AUTO-LINKER & VALIDATOR v2.0      ")
print("="*50 + "\n")
...
# Convert paragraphs to list so we can count them for the progress bar
...
# --- PHASE 1: MAPPING ---
print("\n[*] Phase 1: Scanning Reference List")

for i, p in enumerate(all_paragraphs):
    # Update Animation
    print_progress_bar(i + 1, total_paras, prefix='Scanning:', suffix='Complete', length=30)
    fake_thinking_time() # Artificial delay for coolness
...
for i, p in enumerate(all_paragraphs):
    # Update Animation
    print_progress_bar(i + 1, total_paras, prefix='Linking :', suffix='Complete', length=30)
    fake_thinking_time() # Artificial delay
...
print(f" [+] Citations Linked : {len(linked_references)}")
